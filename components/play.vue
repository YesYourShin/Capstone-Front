<template>
  <div class="gamebox">
    ...
    ref="timer" class="timerbox"></Timer>
...
export default {
  ...
  data() {
    return {
      ...
      userName : ['유저1', '유저2', '유저3', '유저4', '유저5', '유저6', '유저7', '유저8', '유저9', '유저10'], // 각 유저의 닉네임(더미)
  ...
  methods: {
    ...
    startVote() {
      ...
      this.flowMessage = '해가 저물어갑니다. 밤이 되기 전, 마피아로 의심되는 유저를 지목합니다'
      this.$refs.timer.startVote()
    ...
    finishVote() {
      ...
      for (let i = 0; i < this.playerVote.length; i++) {
        if(this.survivePlayer[i] == true) {
          ...
          this.playerVote.splice(this.voteNum-1,1,this.playerVote[this.voteNum-1]+1)
    ...
    finishPunishmentVote() {
      ...
      if (this.punishmentPros > this.punishmentCons) {
        ...
        this.flowMessage = '투표 결과 ' + (this.electedPlayer+1) + ' 를 사형합니다.'
        console.log(this.survivePlayer)
        console.log('마피아는' + this.mafiaNum)
        console.log('시민은' + this.citizenNum)
        if (this.playerJob[this.electedPlayer] == '마피아' && this.survivePlayer[this.electedPlayer] == false) {
          this.mafiaNum = this.mafiaNum - 1
        } else if (this.playerJob[this.electedPlayer] != '마피아' && this.survivePlayer[this.electedPlayer] == false) {
        ...
        //       }
        //     }
            if (this.mafiaNum == 0 || this.citizenNum == 0 || this.mafiaNum >= this.citizenNum) {
              this.victorySearch()
            } else {
              setTimeout(() => {
                this.flowMessage = '밤이 되었습니다. 능력을 사용합니다.'
    ...
    nightSkillEvent() {
        ...
        if (this.survivePlayer[this.killPlayer-1] == false) {
          this.killPlayer = prompt('죽일 유저 다시 선택');
            ...
            if(this.doctorSelected == this.killPlayer && this.killPlayer != this.survivePlayer[this.killPlayer-1]) {
                this.flowMessage = '마피아가 ' + this.killPlayer + '를 공격하였으나 의사의 도움으로 플레이어는 생존하였습니다.'
                this.electedPlayersNum = 1;
    ...
    mafiaWin() {
      this.flowMessage = '마피아가 남아있는 시민의 수와 같거나 많습니다. 마피아가 승리하였습니다.'
    },
    citizenWin() {
